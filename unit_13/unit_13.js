...
function f8() {
  if (a8[i8.value] !== undefined) {
    document.querySelector(".out-8").innerHTML = a8[i8.value];
  } else {
...
function f19() {
  let out = "";
  let station = i19.value.toLowerCase();
  for (let branch in a19) {
    for (let i = 0; i < a19[branch].length; i++) {
      if (a19[branch][i].toLowerCase() === station) {
        out = branch;
      }
    }
  }
...
function f20() {
  let out = "";
  for (branch in a20) {
    for (let i = 0; i < a20[branch].length; i++) {
      if (a20[branch][i][1] === 2) out += a20[branch][i][0] + " ";
    }
  }
