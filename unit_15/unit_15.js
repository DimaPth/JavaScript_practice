...
const f7 = () => {
  let out = 0;
  let password = Array.from(i7.value);
  let uniquePassword = new Set(password);
  if (password.length === uniquePassword.size && password.length > 6) {
    out = 1;
  }
...
const f10 = (out_set, elem) => {
  let result = Array.from(out_set).join(" ");
  document.querySelector(elem).innerHTML = result;
};
